...
//   user_experience_to_level_up = user_level * 25;
  ...
  enemy_health_bar_inner = layer_create(GRect(12, 12, 50, 10));
  layer_add_child(window_layer, enemy_health_bar_inner);
  layer_set_update_proc(enemy_health_bar_inner, draw_enemy_health_bar);
  
  //Enemy image
  int rand_monster = rand() % 4;
  switch(rand_monster)
  {
    case 1: 
    ...
    break;
    case 3:
    enemy_img = gbitmap_create_with_resource(RESOURCE_ID_HARPY);
    break;
    default:
